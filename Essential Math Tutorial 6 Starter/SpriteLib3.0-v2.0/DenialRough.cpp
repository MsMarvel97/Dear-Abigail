...
void DenialRough::InitScene(float windowWidth, float windowHeight)
{
	...
	//Area Trigger 1 (area in which shadow 1 will shoot at player)
	{
		//Creates entity
		auto entity = ECS::CreateEntity();
		...
		ECS::GetComponent<Transform>(entity).SetPosition(vec3(80.f, 30.f, 5.f));
	...
	//Area Trigger 2 (area in which shadow 2 will shoot at player)
	{
		//Creates entity
		auto entity = ECS::CreateEntity();
		...
		ECS::GetComponent<Transform>(entity).SetPosition(vec3(180.f, 30.f, 5.f));
		ECS::GetComponent<Trigger*>(entity) = new ShadowAreaTrigger();
		ECS::GetComponent<Trigger*>(entity)->SetTriggerEntity(entity);
		ECS::GetComponent<Trigger*>(entity)->AddTargetEntity(player);
		ECS::GetComponent<Trigger*>(entity)->SetShadowZone(2);
		ECS::GetComponent<Sprite>(entity).SetTransparency(0.5f);
...
void DenialRough::Update()
{
	...
	auto& shadowThing = ECS::GetComponent<ShadowSense>(player);
	...
	DenialRough::ActivateShadow();
}

void DenialRough::KeyboardHold()
{
	auto& player = ECS::GetComponent<PhysicsBody>(MainEntities::MainPlayer());
	auto& work = ECS::GetComponent<ShadowSense>(MainEntities::MainPlayer());
	float speed = 1.f;
	...
	if (Input::GetKey(Key::G)) //for debugging purposes
	{
		std::cout << work.getLoc();
	}
...
void DenialRough::KeyboardDown()
{
	...
	//if (Input::GetKeyDown(Key::Y)) //for debugging purposes
	//{
	//	DenialRough::SpawnBullet(shadow1);
...
void DenialRough::SpawnBullet(int shadowEntity) //spawns a bullet at the center of shadowEntity and sets it as a static body
{
...
void DenialRough::ShootBullet() //changes the bullet's physics body to a dynamicBody and sets linear velocity in the direction of the player
{
...
void DenialRough::ActivateShadow() 
{
	...
	if (theTime.fire == true)
	{
		...
		else if (theLoc.getLoc() == 2)
		{
			DenialRough::SpawnBullet(shadow2);
			DenialRough::ShootBullet();
		}
