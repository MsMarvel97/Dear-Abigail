...
<<<<<<< HEAD
	...
	if (Trigger::GetSpike() == true)
	{
		if (ECS::GetComponent<PlayerMechanics>(m_targetEntities[0]).GetShield() == true)
		{
			...
			ECS::GetComponent<PhysicsBody>(m_targetEntities[0]).GetBody()->SetLinearVelocity(b2Vec2(10000000 * normalX, 10000000 * normalY));
		}
		else
		{
			ECS::GetComponent<PlayerMechanics>(m_targetEntities[0]).SetCanMove(false);
