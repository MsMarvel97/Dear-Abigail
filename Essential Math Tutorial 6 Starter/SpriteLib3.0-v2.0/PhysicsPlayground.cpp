...
void PhysicsPlayground::InitScene(float windowWidth, float windowHeight)
{
	...
	//button for top level
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(674.5f), float32(40.5f));
	...
	//Setup receptacle destroy trigger
	{
		...
		ECS::GetComponent<Trigger*>(entity)->AddTargetEntity(nachoCeiling);
		ECS::GetComponent<Trigger*>(entity)->AddTargetEntity(receptacleRamp);
		ECS::GetComponent<Trigger*>(entity)->AddTargetEntity(nachoButton);
	...
	//PATH TO BOX TRIGGERS\\
		...
		//Setup left wall destroy trigger
		{
			//Creates entity
			auto entity = ECS::CreateEntity();
