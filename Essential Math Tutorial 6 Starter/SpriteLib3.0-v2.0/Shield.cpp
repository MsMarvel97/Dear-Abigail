...
void Shield::InitScene(float windowWidth, float windowHeight)
{
	...
	//projectile trigger
	{
		...
		b2BodyDef tempDef;
		tempDef.type = b2_staticBody;
		tempDef.position.Set(float32(160.f), float32(20.f));

		tempBody = m_physicsWorld->CreateBody(&tempDef);

		tempPhsBody = PhysicsBody(entity, tempBody, float(tempSpr.GetWidth() - shrinkX),
			float(tempSpr.GetHeight() - shrinkY), vec2(0.f, 0.f), true, TRIGGER, PLAYER);
		tempPhsBody.SetColor(vec4(2.f, 1.f, 0.f, 0.3f));
	}
...
void Shield::Update()
{
	...
	Shield::ReattachCamera();
}

void Shield::KeyboardHold()
{
...
void Shield::KeyboardDown()
{
	...
	if (Input::GetKeyDown(Key::Enter) && shieldMech.restart == true) //checks to see if the shield cooldown if complete before reactivating shield
	{
		shieldMech.setSequenceStart(true);
	}
}

void Shield::KeyboardUp()
{

}

void Shield::ReattachCamera()
{
	ECS::GetComponent<HorizontalScroll>(MainEntities::MainCamera()).SetFocus(&ECS::GetComponent<Transform>(MainEntities::MainPlayer()));
	ECS::GetComponent<VerticalScroll>(MainEntities::MainCamera()).SetFocus(&ECS::GetComponent<Transform>(MainEntities::MainPlayer()));
}

void Shield::CheckShield()
{
